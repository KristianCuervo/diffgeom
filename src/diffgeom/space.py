...
class Space:
    def __init__(self, metric:sp.Matrix, coord_vars:sp.Matrix):
        """
        A Riemannian manifold given directly by its metric.
        coord_vars = coordinates on the manifold
        metric = metric matrix G of the manifold
        """

        self.vars = coord_vars
        self.dim = len(coord_vars)
        self.metric = metric
...
class EuclideanSpace(Space):
    def __init__(self, n:int):
        ...
        super().__init__(metric=sp.eye(n), coord_vars=coord_vars)
